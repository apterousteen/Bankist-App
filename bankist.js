...
const formatMovementDate = (acc, i) => {
    let movementDate;
    let daysPassed = calcDaysPassed(new Date(), new Date(acc.movementsDates[i][0]));
    switch (true) {
        case daysPassed === 0:
            movementDate = 'Today';
            break;
        ...
        case (daysPassed <= 3):
            ...
            break;
        default:
            movementDate = new Date(acc.movementsDates[i][0]).toLocaleDateString(acc.locale);
            break;
    }

    return `${movementDate} ${acc.movementsDates[i][1] ? `from ${acc.movementsDates[i][1]}` : ''}`;
};
...
const transferMoney = (e) => {
    ...
    if (recipientAcc && recipientAcc?.username !== curAccount.username) {
        if (amount > 0 && curAccount.balance >= amount) {
            ...
            recipientAcc.movementsDates.push([new Date().toISOString(), curAccount.owner.split(' ')[0]]);
